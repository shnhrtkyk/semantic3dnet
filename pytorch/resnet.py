    ...
    def __init__(self, numChannels, batch_norm=False, elu=False):
        ...
        if (not elu) and (not batch_norm):
            ...
            self.branch0 = nn.Sequential(
                nn.Conv3d(numChannels, int(numChannels / 2), kernel_size=3, stride=2, padding=1),
...
class VoxceptionNet(nn.Module):
    def __init__(self):
        ...
        self.b1 = nn.Sequential(
            Resdrop(nn.Sequential(nn.BatchNorm3d(32, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=32)),
                    0.95),
            Resdrop(nn.Sequential(nn.BatchNorm3d(32, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=32)),
                    0.90),
            Resdrop(nn.Sequential(nn.BatchNorm3d(32, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=32)),
        ...
        self.b2 = nn.Sequential(
            Resdrop(nn.Sequential(nn.BatchNorm3d(64, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=64)),
                    0.7),
            Resdrop(nn.Sequential(nn.BatchNorm3d(64, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=64)),
                    0.6),
            Resdrop(nn.Sequential(nn.BatchNorm3d(64, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=64)),
        ...
        self.b3 = nn.Sequential(
            Resdrop(nn.Sequential(nn.BatchNorm3d(128, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=128)),
                    0.5),
            Resdrop(nn.Sequential(nn.BatchNorm3d(128, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=128)),
                    0.45),
            Resdrop(nn.Sequential(nn.BatchNorm3d(128, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=128)),
        ...
        self.b4 = nn.Sequential(
            Resdrop(nn.Sequential(nn.BatchNorm3d(256, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=256)),
                    0.35),
            Resdrop(nn.Sequential(nn.BatchNorm3d(256, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=256)),
                    0.30),
            Resdrop(nn.Sequential(nn.BatchNorm3d(256, eps=0.001), nn.ELU(), Voxception_Resnet_Layer(numChannels=256)),
        ...
        self.b5 = nn.Sequential(Resdrop(nn.Sequential(nn.Conv3d(512, 512, 3, 1, 1), nn.BatchNorm3d(512, eps=0.001)), 0.5),
                            nn.ELU())
...
class ResidualSemantic3DNet(nn.Module):
    def __init__(self, n_classes=2, num_scale = 4):
        ...
        concate_layers = [
            nn.Conv3d(512 * num_scale, 512, 2, stride=2),
            nn.BatchNorm3d(512, eps=1e-4),
...
if __name__ == '__main__':
    backbone_net = ResidualSemantic3DNet().cuda()

    print(backbone_net)
    backbone_net.eval()
    out = backbone_net(torch.rand(4, 2, 1, 32, 32, 32).cuda())
